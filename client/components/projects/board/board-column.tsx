...
export default function BoardColumn({
  ...
  return (
    <DroppableColumn id={id}>
      <div
        id={id}
        className="flex h-full min-w-[250px] max-w-[250px] flex-col rounded-xl border bg-muted/30"
      >
        ...
        <ScrollArea className="flex-1">
          <div
            className={cn(
              ...
              taskCount === 0 && "items-center justify-center",
            )}
          >
            {taskCount > 0 ? (
              children
            ) : (
              <EmptyColumn onCreateTask={onCreateTask} />
            )}
          </div>
        </ScrollArea>
      </div>
...
function EmptyColumn({ onCreateTask }: EmptyColumnProps) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      <div className="mb-4 rounded-full bg-muted p-4" onClick={onCreateTask}>
